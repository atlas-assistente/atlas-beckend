...
// ===== LOGIN CLIENTE (ADICIONADO) =====
const API_CLIENT = "https://atlas-beckend.onrender.com/api";

const clientBox = document.getElementById("atlas-client-login");
...
const clientCodeBox = document.getElementById("clientCodeBox");
const clientMsg = document.getElementById("clientMsg");

// mostra login do cliente por padrão se não houver sessão
if (!localStorage.getItem("atlas_token")) {
  if (clientBox) clientBox.style.display = "block";
}

if (btnClientSend) btnClientSend.onclick = async () => {
  const r = await fetch(API_CLIENT + "/auth/request", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ email: clientEmail.value })
  });
  const d = await r.json();
  if (!r.ok) return (clientMsg.innerText = d.error || "Erro");
  clientMsg.innerText = "Código enviado";
  clientCodeBox.style.display = "block";
};

if (btnClientLogin) btnClientLogin.onclick = async () => {
  const r = await fetch(API_CLIENT + "/auth/verify", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
